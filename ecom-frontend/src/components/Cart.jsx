        ...
        const cartItemsWithImages = await Promise.all(
          updatedCartItems.map(async (item) => {
            try {
              ...
              const imageFile = await converUrlToFile(imgResponse.data, productMeta?.imageName || "image");
              setCartImage(imageFile);
      ...
      <div className="shopping-cart">
        ...
        {cartLoading && cart.length > 0 ? (
          <div className="cart-loading" aria-busy="true">
            <div className="skeleton skeleton-title" style={{ width: "60%", height: 24, margin: "1rem 0" }} />
            <div className="skeleton" style={{ height: 80, marginBottom: 12 }} />
            <div className="skeleton" style={{ height: 80, marginBottom: 12 }} />
          </div>
        ) : cartItems.length === 0 ? (
          <div className="empty cart-empty-state">
            <i className="bi bi-cart-x cart-empty-icon" aria-hidden />
            <h4>Your cart is empty</h4>
          ...
          <>
            {cartItems.map((item) => (
              <li key={item.id} className="cart-item">
                ...
                >
                  ...
                  <div className="total-price " style={{ textAlign: "center" }}>
                    <i className="bi bi-currency-rupee" aria-hidden />{item.price * item.quantity}
                  </div>
                  <button
                    className="remove-btn"
                    onClick={() => handleRemoveFromCart(item.id)}
            ...
            <div className="total">Total: <i className="bi bi-currency-rupee" aria-hidden />{totalPrice.toFixed(2)}</div>
            <Button
              className="btn btn-primary"
              style={{ width: "100%" }}
